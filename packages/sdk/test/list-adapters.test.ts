...
test('listAdapters reports one entry per built-in adapter', async () => {
  await withIsolatedHome(() =>
    withBins({ CLAUDE_BIN: '/no/claude', OPENCODE_BIN: '/no/opencode', CODEX_BIN: '/no/codex' }, () => {
      ...
      assert.equal(adapters.every((a) => !a.detected), true);
      assert.deepEqual(
...
test('listAdapters marks every detected adapter in a multi-marker project', async () => {
  await withIsolatedHome(() =>
    withBins({ CLAUDE_BIN: FAKE_CLAUDE, OPENCODE_BIN: FAKE_OPENCODE, CODEX_BIN: FAKE_CODEX }, () => {
      ...
      assert.deepEqual(
        sc.listAdapters().filter((a) => a.detected).map((a) => a.kind),
        ['claude-code', 'opencode', 'codex'],
      );
    }),
  );
...
test('listAdapters selected flag follows config and setAdapter', async () => {
  await withIsolatedHome(() =>
    withBins({ CLAUDE_BIN: '/no/claude', OPENCODE_BIN: '/no/opencode', CODEX_BIN: '/no/codex' }, () => {
      ...
      assert.deepEqual(
        sc.listAdapters().filter((a) => a.selected).map((a) => a.kind),
        ['opencode'],
      );
...
test('listAdapters reports no selected adapter when none resolves', async () => {
  await withIsolatedHome(() =>
    withBins({ CLAUDE_BIN: '/no/claude', OPENCODE_BIN: '/no/opencode', CODEX_BIN: '/no/codex' }, () => {
      ...
      assert.equal(adapters.some((a) => a.selected), false);
      assert.equal(adapters.some((a) => a.detected), false);
    }),
  );
});
