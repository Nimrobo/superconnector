...
test('recordSpawn groups selectors under one app entry in v2 registry', () => {
  ...
  const raw = JSON.parse(readFileSync(p.file, 'utf8')) as {
    ...
    apps: Array<{
      ...
      appId: string;
      adapter?: string;
      sessions: Array<{ sessionId: string; sessionSelector?: string }>;
    }>;
  };
  ...
  assert.deepEqual(
    raw.apps[0]!.sessions.map((s) => [s.sessionId, s.sessionSelector]),
    [
      ['sess-a', 'thread-a'],
      ['sess-b', 'thread-b'],
    ],
...
test('listSessions can list all app sessions or narrow by sessionSelector', () => {
  ...
  recordSpawn(
    { cwd, appId: 'app', adapter: 'claude-code', sessionId: 'a', sessionSelector: 'thread-a' },
    p,
  );
  recordSpawn(
    { cwd, appId: 'app', adapter: 'claude-code', sessionId: 'b', sessionSelector: 'thread-b' },
    p,
  );
  recordSpawn(
    {
  ...
  assert.deepEqual(
    listSessions({ cwd, appId: 'app', sessionSelector: 'thread-a' }, p).map((s) => s.sessionId),
    ['a'],
  );
  assert.deepEqual(
    listSessions({ cwd, sessionSelector: 'thread-a' }, p)
      .map((s) => s.sessionId)
      .sort(),
    ['a', 'other'],
  );
...
test('recordResume only enforces sessionSelector when provided', () => {
  ...
  recordSpawn(
    { cwd, appId: 'app', adapter: 'claude-code', sessionId: 'sess-r', sessionSelector: 'thread-a' },
    p,
  );

  assert.ok(recordResume({ cwd, appId: 'app', sessionId: 'sess-r' }, p));
  assert.ok(
    recordResume({ cwd, appId: 'app', sessionId: 'sess-r', sessionSelector: 'thread-a' }, p),
  );
  assert.equal(
    recordResume({ cwd, appId: 'app', sessionId: 'sess-r', sessionSelector: 'thread-b' }, p),
    null,
  );
});
...
test('findLatestSession is scoped to the provided selector and defaults to unselected sessions', async () => {
  ...
  assert.equal(
    findLatestSession({ cwd, appId: 'app', sessionSelector: 'thread-a' }, p)?.sessionId,
    'selector-new',
  );
