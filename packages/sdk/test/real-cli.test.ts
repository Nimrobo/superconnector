...
for (const c of cases) {
  test(`${c.name} real CLI contract`, { skip: !enabled || !isExecutableAvailable(configuredBin(c)) }, async (t) => {
    ...
    await t.test('optional live spawn emits a session id and terminal event', {
      ...
      try {
        const messages = await drainUntilResult(adapter.spawn({
